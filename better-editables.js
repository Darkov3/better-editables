(function(){
	...
	// betterEditableData scope
	{
		...
		$.betterEditableData.methods = {
			...
			validate: function (editable, triggerErrors, value) {
				if (triggerErrors !== true) {
					triggerErrors = false;
				}
				return editable.validate(triggerErrors, value);
			},
		...
		$.betterEditableData.lastAutoGeneratedId = 0;
		...
		$.betterEditableData.functions = {
			...
			applyToAll: function () {
				...
				if (typeof argumentArray[0] === 'function'){
					condition = argumentArray.shift();
				}
	...
	// Initialization scope
	{
		window.BetterEditable = function(settings) {
			...
			if (typeof this.$element.attr('id') !== 'string' || this.$element.attr('id').trim() == "") {
				this.$element.attr('id', "betterEditableAutoGeneratedId-" + $.betterEditableData.lastAutoGeneratedId)
				++$.betterEditableData.lastAutoGeneratedId;
			}
